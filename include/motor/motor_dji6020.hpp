...
class Motor<MotorType::DJI_6020> {
    ...
    Motor(const Config& config, bool to_register = true);
    ...
    static void motor_6020_pack(MotorFdb& msg, const std::vector<uint8_t>& data, uint32_t id, real max_current) {
        msg.id = id;
        ...
        msg.current.num = (real)(short)(((short)data[4]) << 8 | data[5]) * max_current / 16384.0;
...
Motor<MotorType::DJI_6020>::Motor(const Config& config, bool to_register) : 
